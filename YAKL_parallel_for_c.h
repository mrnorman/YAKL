...
namespace c {
  ...
  template <class F, bool simple> YAKL_INLINE void callFunctor(F const &f , Bounds<8,simple> const &bnd , int const i ) {
  ...
  #ifdef YAKL_ARCH_CUDA
    template <class F, int N, bool simple> __global__ void cudaKernelRef( Bounds<N,simple> bounds , F const &dummy ) {
      size_t i = blockIdx.x*blockDim.x + threadIdx.x;
      const F &f = *((const F*)functorBuffer);
      if (i < bounds.nIter) {
    ...
    void parallel_for_cuda( Bounds<N,simple> const &bounds , F const &f , int vectorSize = 128 ) {
      cudaMemcpyToSymbolAsync(functorBuffer,&f,sizeof(F),0,cudaMemcpyHostToDevice);
      check_last_error();
      cudaKernelRef <<< (unsigned int) (bounds.nIter-1)/vectorSize+1 , vectorSize >>> ( bounds , f );
      check_last_error();
    }
  #endif
