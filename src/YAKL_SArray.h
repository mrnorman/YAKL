...
namespace yakl {
  ...
  consteval bool valid_sarray_product() {
    uindex_t product = 1;
    for (uindex_t extent : {static_cast<uindex_t>(DIMS)...}) {
  ...
  class SArray {
    ...
    KOKKOS_INLINE_FUNCTION T & operator()(std::integral auto... indices) const {
      ...
      uindex_t idx[rank] = {checked_uindex(indices,"ERROR: SArray index is not representable by uindex_t")...};
      uindex_t constexpr dims[rank] = {static_cast<uindex_t>(DIMS)...};
    ...
    KOKKOS_INLINE_FUNCTION auto unpack_global_index(std::integral auto iglob) const {
      ...
      for (int i=0; i < rank; i++) {
        ret(i) = (static_cast<uindex_t>(iglob) / offsets[i]) % dims[i];
      }
      return ret;
    }
