...
namespace yakl {
  namespace intrinsics {
    ...
    // SIGN
    template <class ViewType> requires (yakl::is_SArray<ViewType> || std::is_arithmetic_v<ViewType>)
    KOKKOS_INLINE_FUNCTION ViewType sign( ViewType const & a , ViewType const & b ) {
    ...
    // MERGE
    template <class V1, class V2> requires ( (yakl::is_SArray     <V1> && yakl::is_SArray     <V2>) ||
                                             (std::is_arithmetic_v<V1> && std::is_arithmetic_v<V2>) )
    KOKKOS_INLINE_FUNCTION V1 merge(V1 const & t, V1 const & f, V2 const & cond) {
    ...
    // MATMUL_RC
    template <class V1, class V2 >requires yakl::is_SArray<V1> && yakl::is_SArray<V2> && V1::is_cstyle && V2::is_cstyle
    ...
    // ANY
    template <class ViewType> requires yakl::is_SArray<ViewType>
    KOKKOS_INLINE_FUNCTION bool any(ViewType const & in) {
      bool any_true = false;
    ...
    // ALL
    template <class ViewType> requires yakl::is_SArray<ViewType>
    KOKKOS_INLINE_FUNCTION bool all(ViewType const & in) {
      bool all_true = true;
    ...
    // SUM
    template <class ViewType> requires yakl::is_SArray<ViewType>
    KOKKOS_INLINE_FUNCTION typename ViewType::non_const_value_type sum(ViewType const & in) {
    ...
    // PRODUCT
    template <class ViewType> requires yakl::is_SArray<ViewType>
    KOKKOS_INLINE_FUNCTION typename ViewType::non_const_value_type product(ViewType const & in) {
      using scalar_t = typename ViewType::non_const_value_type;
      scalar_t result = in.data()[0];
      for (int i=1; i < in.size(); i++) { result *= in.data()[i]; }
      return result;
    ...
    // MINVAL
    template <class ViewType> requires yakl::is_SArray<ViewType>
    KOKKOS_INLINE_FUNCTION typename ViewType::non_const_value_type minval(ViewType const & in) {
      using scalar_t = typename ViewType::non_const_value_type;
      scalar_t result = in.data()[0];
    ...
    // MAXVAL
    template <class ViewType> requires yakl::is_SArray<ViewType>
    KOKKOS_INLINE_FUNCTION typename ViewType::non_const_value_type maxval(ViewType const & in) {
