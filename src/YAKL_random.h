...
namespace yakl {
  ...
  public:
    ...
    KOKKOS_INLINE_FUNCTION void set_seed(u8 seed, u8 stream_id) {
      ...
      state.output0                = 0;
      state.output1                = 0;
      state.normal_spare           = 0;
      state.next                   = 2;
      state.normal_spare_available = false;
    }

    /** @brief Generates either a raw uint64_t or a floating-point value in [0,1). */
    template <class T = float> requires (std::is_floating_point_v<T> || std::is_same_v<T,u8>)
    KOKKOS_INLINE_FUNCTION T gen_uniform() {
      if constexpr (std::is_same_v<T,u8>) {
        ...
        static_assert(digits <= 64,"Random::gen_uniform supports floating-point types with at most 64 mantissa bits");
        if constexpr (digits < 64) {
          return static_cast<T>(gen_uniform<u8>() >> (64-digits)) / static_cast<T>(u8(1) << digits);
        } else {
          return static_cast<T>(gen_uniform<u8>()) * static_cast<T>(0x1p-64L);
        }
      }
    }
    ...
    /** @brief Generates a normally distributed value with the requested mean and standard deviation.
      *
      * Box-Muller produces two values at a time. CacheSpare retains the second value by default. Setting it to false
      * discards both an existing spare and the second value generated by this call, giving fixed two-uniform consumption.
      */
    ...
    KOKKOS_INLINE_FUNCTION T gen_normal(T mean = T{0}, T stddev = T{1}) {
      ...
      if constexpr (CacheSpare) {
        if (state.normal_spare_available) {
          state.normal_spare_available = false;
          return mean + stddev*static_cast<T>(state.normal_spare);
        }
      } else {
        state.normal_spare_available = false;
      }

      double const radial_uniform = 1.0-gen_uniform<double>();
      double const angle_uniform  = gen_uniform<double>();
      double constexpr two_pi = 6.283185307179586476925286766559;
      double const radius = std::sqrt(-2.0*std::log(radial_uniform));
      double const angle  = two_pi*angle_uniform;
      double const normal = radius*std::cos(angle);
      if constexpr (CacheSpare) {
        state.normal_spare = radius*std::sin(angle);
        state.normal_spare_available = true;
      }
      return mean + stddev*static_cast<T>(normal);
    }
    ...
    KOKKOS_INLINE_FUNCTION bool gen_bernoulli(double probability = 0.5) {
      ...
      return gen_uniform<double>() < probability;
    }

    /** @brief Generates an exponentially distributed value with the requested positive rate. */
    ...
    /** @brief Generates a lognormally distributed value from the requested underlying normal parameters. */
    template <class T = float, bool CacheSpare = true> requires std::is_floating_point_v<T>
    KOKKOS_INLINE_FUNCTION T gen_lognormal(T normal_mean = T{0}, T normal_stddev = T{1}) {
      return std::exp(gen_normal<T,CacheSpare>(normal_mean,normal_stddev));
    }
  };
