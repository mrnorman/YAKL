...
namespace yakl {
  ...
  //   * Operations that mutate allocation metadata take exclusive ownership of mutex. Read-only operations share it.
  ...
  public:
    ...
    // The allocation list is the frequently accessed mutable state. A shared mutex lets queries proceed concurrently while
    // allocations and frees retain exclusive access to the address-ordered vector.
    mutable std::shared_mutex             mutex;


    LinearAllocator() { nullifyUnlocked(); }
    ...
    LinearAllocator( size_t                                bytes ,
      ...
      if (poolSizeUnlocked() > std::numeric_limits<size_t>::max()-(requiredAlignment-1)) {
        Kokkos::abort("ERROR: LinearAllocator aligned pool-size overflow");
      }
      this->poolAllocation = mymalloc( poolSizeUnlocked()+requiredAlignment-1 );
      ...
      this->myzero( pool , poolSizeUnlocked() );
    }
    ...
    void * allocate(size_t bytes, std::string label="") {
      ...
      if constexpr (kokkos_debug) {
        if (pool == nullptr) Kokkos::abort("ERROR: allocating from an uninitialized LinearAllocator");
      }
      if (bytes > std::numeric_limits<size_t>::max()-(blockSize-1)) {
      ...
      if (allocs.empty()) {
        ...
        if ( allocs.front().start >= blocksReq ) {
          ...
          return getPtrUnlocked(allocs[0].start);
        }
        ...
        for (size_t i=0; i+1 < allocs.size(); i++) {
          if ( allocs[i+1].start - (allocs[i].start + allocs[i].length) >= blocksReq ) {
            ...
            return getPtrUnlocked(allocs[i+1].start);
          }
        }
        ...
        if ( nBlocks - (allocs.back().start + allocs.back().length) >= blocksReq ) {
          ...
          return getPtrUnlocked(allocs.back().start);
        }
      }
    ...
    size_t free(void * ptr, std::string label = "") {
      ...
      for (size_t i=0; i < allocs.size(); i++) {
        if (ptr == getPtrUnlocked(allocs[i].start)) {
    ...
    bool iGotRoom( size_t bytes ) const {
      ...
      std::shared_lock<std::shared_mutex> lock(mutex);
      if constexpr (kokkos_debug) {
        if (pool == nullptr) Kokkos::abort("ERROR: querying an uninitialized LinearAllocator");
      }
    ...
    bool thisIsMyPointer(void * ptr_in) const {
      ...
      return ptr >= start && ptr-start < poolSizeUnlocked();
    }
    ...
    void * getPtr( size_t blockIndex ) const {
      std::shared_lock<std::shared_mutex> lock(mutex);
      return getPtrUnlocked(blockIndex);
    }


  private:

    void nullifyUnlocked() {
    ...
    void printAllocsLeftUnlocked() const {
      if (allocs.size() != 0) {
        ...
        for (size_t i=0; i < allocs.size(); i++) {
          std::cerr << "*** Label: "         << allocs[i].label
                    ...
                    << " bytes  ;  ptr: "    << getPtrUnlocked(allocs[i].start) << std::endl;
        }
      }
    }


    size_t poolSizeUnlocked() const {
    ...
    void * getPtrUnlocked( size_t blockIndex ) const {
