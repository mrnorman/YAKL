...
namespace simd {
  ...
  public:
    ...
    //////////////////////////////////////
    template <class TLOC>
    KOKKOS_INLINE_FUNCTION Pack<T,N> & operator+= (Pack<TLOC,N> rhs) {
      GET_SIMD_PRAGMA()
    ...
    inline friend std::ostream &operator<<(std::ostream& os, Pack<T,N> const &v) {
      for (size_t i=0; i<N; i++) { os << std::setw(12) << v(i) << "  "; }
      os << "\n";
      return os;
    }
  };
   ...
   */
  template <size_t N, bool SIMD=false> struct PackIterConfig {};
   ...
   * IMPORTANT: For the functor passed to this routine, please use **[&]** syntax, not KOKKOS_LAMBDA
   ...
   *               SIMD pragma.
   */
  template <class F, size_t N, bool SIMD=false>
  KOKKOS_INLINE_FUNCTION void iterate_over_pack( F const &f , PackIterConfig<N,SIMD> config ) {
    if constexpr (SIMD) {
      GET_SIMD_PRAGMA()
  ...
  //////////////////////////////////////////////////////////////
  template <class T, size_t N, class TLOC , typename std::enable_if<std::is_arithmetic<TLOC>::value,bool>::type = false >
  KOKKOS_INLINE_FUNCTION Pack<T,N> operator+ (Pack<T,N> lhs , TLOC val) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = lhs(i) + val; }
    return ret;
  }
  template <class T, size_t N, class TLOC , typename std::enable_if<std::is_arithmetic<TLOC>::value,bool>::type = false >
  KOKKOS_INLINE_FUNCTION Pack<T,N> operator+ (TLOC val , Pack<T,N> rhs) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = val + rhs(i); }
    return ret;
  }
  ...
  template <class T, size_t N, class TLOC , typename std::enable_if<std::is_arithmetic<TLOC>::value,bool>::type = false >
  KOKKOS_INLINE_FUNCTION Pack<T,N> pow(Pack<T,N> lhs , TLOC val) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = std::pow( lhs(i) , val ); }
    return ret;
  }
  ...
  //////////////////////////////////////////////////////////////
  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> operator- ( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
  ...
  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> sqrt( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = std::sqrt( a(i) ); }
    return ret;
  }

  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> abs( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
  ...
  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> exp( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = std::exp( a(i) ); }
    return ret;
  }

  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> log( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = std::log( a(i) ); }
    return ret;
  }

  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> log10( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = std::log10( a(i) ); }
    return ret;
  }

  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> cos( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = std::cos( a(i) ); }
    return ret;
  }

  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> sin( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = std::sin( a(i) ); }
    return ret;
  }

  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> tan( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = std::tan( a(i) ); }
    return ret;
  }

  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> acos( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = std::acos( a(i) ); }
    return ret;
  }

  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> asin( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = std::asin( a(i) ); }
    return ret;
  }

  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> atan( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = std::atan( a(i) ); }
    return ret;
  }

  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> ceil( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
  ...
  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> floor( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
  ...
  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> round( Pack<T,N> a ) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) { ret(i) = std::round( a(i) ); }
    return ret;
  }
  ...
  //////////////////////////////////////////////////////////////
  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> operator+( Pack<T,N> a , Pack<T,N> b) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
  ...
  template <class T, size_t N>
  KOKKOS_INLINE_FUNCTION Pack<T,N> pow( Pack<T,N> a , Pack<T,N> b) {
    Pack<T,N> ret;
    GET_SIMD_PRAGMA()
    for (size_t i=0; i < N; i++) {
