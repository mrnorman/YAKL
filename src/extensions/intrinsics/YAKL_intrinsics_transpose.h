...
namespace yakl {
  namespace intrinsics {
    ...
    inline Array<T,2,myMem,styleC> transpose(Array<T,2,myMem,styleC> const &in, Stream stream = Stream() ) {
      #ifdef YAKL_DEBUG
    ...
    template <class T, int myMem>
    inline Array<T,2,myMem,styleFortran> transpose(Array<T,2,myMem,styleFortran> const &in, Stream stream = Stream() ) {
      #ifdef YAKL_DEBUG
        if (!allocated(in)) yakl_throw("ERROR: Calling transpose on unallocated array");
      #endif
      auto l1 = lbound(in,1);
