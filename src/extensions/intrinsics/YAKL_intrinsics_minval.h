...
namespace yakl {
  namespace intrinsics {
    ...
    inline T minval( Array<T,rank,memHost,myStyle> const &arr ) {
        ...
        if (!arr.initialized()) { Kokkos::abort("ERROR: calling minval on an array that has not been initialized"); }
      #endif
    ...
    inline T minval( Array<T,rank,memDevice,myStyle> const &arr ) {
      ...
      TNC result;
      Kokkos::parallel_reduce( YAKL_AUTO_LABEL() , arr.size() , KOKKOS_LAMBDA (int i, TNC & lmin) {
        TNC val = arr.data()[i];
        lmin = val < lmin ? val : lmin;
      }, Kokkos::Min<TNC>(result) );
      return result;
    }

    template <class T, int rank, class D0, class D1, class D2, class D3>
    KOKKOS_INLINE_FUNCTION T minval( FSArray<T,rank,D0,D1,D2,D3> const &arr ) {
      typename std::remove_cv<T>::type m = arr.data()[0];
      for (int i=1; i<arr.totElems(); i++) {
        if (arr.data()[i] < m) { m = arr.data()[i]; }
      }
    ...
    KOKKOS_INLINE_FUNCTION T minval( SArray<T,rank,D0,D1,D2,D3> const &arr ) {
      typename std::remove_cv<T>::type m = arr.data()[0];
      for (int i=1; i<arr.totElems(); i++) {
        if (arr.data()[i] < m) { m = arr.data()[i]; }
      }
