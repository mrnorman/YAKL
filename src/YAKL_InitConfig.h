...
namespace yakl {
  ...
  public:
    InitConfig() : pool_enabled(false) , pool_size_mb(0) , pool_block_bytes(4096) { }
    ...
    InitConfig set_pool_block_bytes( size_t block_bytes) {
      if constexpr (kokkos_debug) {
        if (block_bytes == 0 || block_bytes%(2*sizeof(size_t)) != 0) {
          Kokkos::abort("ERROR: pool block size must be positive and a multiple of 2*sizeof(size_t)");
        }
      }
